...
Function Execute_MSI
{
	try
    {
    	...
    	Write-Output "========="
	Write-Output "InstallType: $InstallType"
	Write-Output "FilePath: $FilePath"
    	Write-Output ""

	if(Test-Path $FilePath) { write-output "MSI exists!" }
	else { throw "MSI does not exist!" }
...
param(
   $Result = $(throw "Please pass the result.")
    ...
    if($Result -ne 0)
    {
       Write-Host "MSI INSTALLATION FAILED:  Please check logfile for errors." -foreground "Red"
       Write-Output "MSI INSTALLATION FAILED:  Please check logfile for errors."
